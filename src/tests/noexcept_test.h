...
namespace noexcept_test
{
	...
	auto add(int32_t a, int32_t b) noexcept -> decltype(a+b)
	{
		return a + b;
	}

	//if we do throw the compiler will warn us (but still compiles + crashes)
	auto div_noexcept(float32_t a, float32_t b) noexcept -> decltype(a/b)
	{
		if (b == 0)
	...
	auto div_except(float32_t a, float32_t b) noexcept(false) -> decltype(a/b)
	{
